...
```bash
touch __init__.py   app.py logix.py cront.py mycronjobs.txt url_list.json
```



```bash
scp -i ./awslighsail.pem /home/caspi/Desktop/masterurlmap.csv ubuntu@13.126.121.104:/home/ubuntu/service5000
```
...
# @app.route('/api0', methods=['GET'])
# def trigger_gen_link0():
#     if request.method == 'GET':
#         stringx = modul()
#         return jsonify({"Key": stringx})


def execute_modul_async():
    thread = Thread(target=modul)
    thread.start()


# API route to trigger module file asynchronously
@app.route('/api0', methods=['GET'])
def trigger_gen_link0():
    if request.method == 'GET':
        execute_modul_async()
        return jsonify({"message": "Module execution started."})


# API route to trigger genLink() function
@app.route('/api1', methods=['POST'])
def trigger_gen_link1():
...
if __name__ == '__main__':
    app.run(port=5000)
# curl -X POST https://cloudforge.cloud/api0
# curl -i -X POST https://cloudforge.cloud/api1
# curl  https://cloudforge.cloud/api2
...
```txt
36 15 * * * python3 app.py > log.txt 2>&1 & 
```
